...
# ============================================================
# MLP + FrameStack baselines on ALL existing MuJoCo POMDP demos
...
STUDENT_DIR="experiments/v2/students_baselines"
TOTAL=0
FAILED=0

echo "=== MLP + FrameStack baselines — $(date) ==="
...
for ENV in halfcheetah_pomdp ant_pomdp; do
    for DEMO_SUBDIR in ${DEMO_DIR}/${ENV}/teacher_*; do
        ...
        for ARCH in mlp framestack; do
            for SEED in 42 123 456; do
                OUTDIR="${STUDENT_DIR}/${ENV}/${DEMO_TAG}/${ARCH}/seed${SEED}"
                if [ -f "${OUTDIR}/result.json" ]; then
                    echo "  SKIP (exists): $ENV / $DEMO_TAG / $ARCH / seed=$SEED"
                    continue
                fi
                echo "  RUN: $ENV / $DEMO_TAG / $ARCH / seed=$SEED"
...
python wasumindV2/evaluation/evaluate_sweep.py \
    ...
    --output experiments/v2/baselines_results.csv
